...
- **Tags auto (#90):** o envelope da destilação (caminho **one-shot**) traz `tags` (1-4 etiquetas curtas, cap 8, normalizadas à Obsidian) que o agente gera REUTILIZANDO as já em uso — a lista é injetada no prompt (`tagsExistentesCom`) para a navegação por tag não virar ruído de variantes. Política **aditiva**: ao CONTINUAR uma nota, `unirTags` funde as novas com as da nota (vindas da candidata), por isso as do utilizador nunca se perdem. Sem chamada CLI extra (espelha o summary). **#95 fechou os 2 follow-ups:** (a) o caminho **agentic** também gera tags — regra no `contract.ts` + `tagsExistentes` no prompt + param `tags` nas tools `criar_nota`/`continuar_nota`; (b) **guard SQL `unir_tags`** nos 3 RPCs de escrita faz a UNIÃO `existing ∪ patch` em vez do `||` que substituía — a escrita do agente nunca esmaga as tags do utilizador (a remoção do user vive noutro RPC, `update_knowledge_properties`, que não une). A política aditiva passa a estar no SQL, não só no TS.
- **Placement por projeto (#96):** o envelope da destilação (one-shot) traz `projeto` — o agente decide o destino da nota nova: trabalho de um projeto → pasta desse projeto (`resolverProjetoCom` + `escreverNotaEmPastaCom`); sobre o utilizador → **Pessoal**; referência do mundo (sem dono) → **Knowledge** (raiz, campo omitido). Ao **CONTINUAR** uma candidata, herda a pasta dela (inalterado). O placement é **best-effort** — o utilizador refina com drag-drop. O caminho **agentic** (`criar_nota`) ainda escreve na raiz (fatia futura).
- **Densidade de links (#104):** a teia saía fina — o agente ligava ~0 wikilinks porque o nudge só vivia no ramo "cria vizinho novo E liga", que nunca disparava ao **continuar**. Fix: a instrução de ligar passa a **incondicional** (liga a TODAS as candidatas relacionadas do turno, ao criar **e** ao continuar) + bullet **ÍNDICE** best-effort (quando o turno junta VÁRIAS notas como partes de um TODO — ex.: "PC novo" + componentes — cria/mantém uma nota-índice que as liga, em vez de enterrar numa parte), com guard anti-clone (não duplicar nem partir um assunto). Espelhado nos dois prompts (`contract.ts` agentic + `chat.turno.ts` one-shot). `candidatosParaFactoCom` sobe o `limite` 3→5 (a estrela precisa de ver os vizinhos; pool já é 8, zero retrieval extra). Medido pelo eval `densidade` (helper `contarLinksResolvidos`): baseline 1/5 PASS (média 0) → pós-fix **5/5, média 4 links/nota com hub de grau 4**. Os backlinks já são derivados dos `edges`, por isso **não** se tece o reverso à mão (seria estrada de dois sentidos, não rede).
