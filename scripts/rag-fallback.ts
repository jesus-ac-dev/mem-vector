...
async function main(): Promise<void> {
    ...
    ok =
        check(
            'conhecimento geral responde (não fica refém do RAG)',
            /lisboa/i.test(geral.text),
            geral.text,
        ) && ok;
    ok =
        check(
    ...
    ok =
        check(
            'facto do workspace presente é recuperado',
            /girafa-4471/i.test(com.text),
            com.text,
        ) && ok;
    ...
    ok =
        check(
            'facto do workspace ausente NÃO é inventado',
            !/girafa-4471/i.test(sem.text),
            sem.text,
        ) && ok;
    ...
    ok =
        check(
            'indexar a pergunta ANTES mete-a nas próprias fontes (o bug)',
            indexadaAntes >= 1,
            `fontes = ${indexadaAntes}`,
        ) && ok;
    ok =
        check(
