...
async function main(): Promise<void> {
    ...
    await escreverNotaCom(db, { title: 'Nota Em Pasta FP', content_md: '# x', links: [], reason: 'p' });
    const upd = await db.from('knowledge').update({ folder_id: pasta.id }).eq('owner_id', userId).eq('slug', slugNota);
    ...
    const arvore = construirArvore(
        pastas,
        notas.map((n) => ({ id: n.id, slug: n.slug, title: n.title, folderId: n.folderId ?? null })),
    );
