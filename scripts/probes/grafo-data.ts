...
async function main(): Promise<void> {
    ...
    const temNos =
        grafo.nodes.some((n) => n.slug === SLUG_GAMA) &&
        grafo.nodes.some((n) => n.slug === SLUG_DELTA);
    console.log(
        `${temNos ? '✅' : '❌'} eixo 1 — nós das duas notas presentes (${grafo.nodes.length} nós)`,
    );
    ...
    const semPendentes = grafo.links.every(
        (l) =>
            grafo.nodes.some((n) => n.id === l.source) &&
            grafo.nodes.some((n) => n.id === l.target),
    );
    console.log(
        `${semPendentes ? '✅' : '❌'} eixo 3 — nenhuma aresta pendente (extremos são nós)`,
    );
