...
| `mcp-tools.ts`        | MCP server stdio lançado pelo claude CLI: `procurar_notas`, `ler_nota`, `criar_nota`, `continuar_nota`, `acrescentar_daily`, `ler_daily_hoje` e (#45) `procurar_web`, `ler_url`. Reutiliza os serviços `...Com` (RPCs transacionais + projeção de índices) sob a **sessão do utilizador** (tokens por env → RLS real, sem service role).                                                                                                                                                       |
...
| `responder-web.ts`    | (#45) Orquestra a **resposta do chat com internet** quando o utilizador liga `webHabilitada` nas Definições: espelha o `destilar-agentic` mas com `allowedTools` só de web (`procurar_web`/`ler_url`). O CLI não faz WebSearch/WebFetch reais em `-p` (inventa, 0 requests) — por isso a web é uma tool MCP nossa (HTTP real em `lib/web.ts`: Tavily com key, DuckDuckGo sem). **Discrição (r3):** o system prompt manda responder primeiro do workspace e só ir à net para factos do mundo — não pesquisa em perguntas do workspace. Devolve texto + `webSources` (URLs do resultado) para a UI mostrar 🌐.                          |
